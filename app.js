import { API_BASE } from "./settings.js?v=20250827a";
...
function stripTags(html) { const tmp = document.createElement("div"); tmp.innerHTML = html || ""; return (tmp.textContent || tmp.innerText || "").trim(); }
...
function isLogoish(url) {
  ...
  return s.includes("logo") || s.includes("favicon") || s.includes("sprite") || s.includes("placeholder")
      || s.includes("default") || s.includes("brand") || s.endsWith(".svg");
}
...
// Resilient fetch: never throws, returns fallback [] by default
...
async function loadHome() {
  ...
  function sectionHtml(feedKey, items) {
    ...
    const fallback = !items.length ? `<ul class="section-grid"><li class="card" style="padding:16px;text-align:center;">No ${FEED_TITLES[feedKey]} yet.</li></ul>` : `<ul class="section-grid">${cards}</ul>`;
    return `
